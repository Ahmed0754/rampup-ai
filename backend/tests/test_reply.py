...
def test_reply_casual_tone(client, auth_headers):
    with patch("app.routers.reply.ClaudeService", return_value=_mock_service("yeah for sure, on it!")):
...
def test_reply_professional_tone(client, auth_headers):
    with patch("app.routers.reply.ClaudeService", return_value=_mock_service("I will have this done today.")):
        response = client.post(
...
def test_reply_manager_safe_tone(client, auth_headers):
    with patch("app.routers.reply.ClaudeService", return_value=_mock_service("Taking ownership of this now.")):
        response = client.post(
...
def test_reply_confused_but_trying_tone(client, auth_headers):
    with patch("app.routers.reply.ClaudeService", return_value=_mock_service("I'm not 100% sure yet, but looking into it.")):
        response = client.post(
            "/api/reply",
...
def test_reply_claude_error_returns_502(client, auth_headers):
    mock = MagicMock()
    mock.generate_reply.side_effect = ClaudeServiceError("AI service unavailable")
    with patch("app.routers.reply.ClaudeService", return_value=mock):
